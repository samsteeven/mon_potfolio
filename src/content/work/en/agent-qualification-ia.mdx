...
stack: ["n8n", "OpenCode Go", "DeepSeek v4 Flash", "Tavily API", "Redis", "Next.js", "Data Tables", "MCP"]
status: "shipped"
featured: true
lang: "en"
...
**OpenCode Go orchestration**: The agent runs on DeepSeek v4 Flash through the **OpenCode Go** gateway (OpenAI-compatible endpoint), the model is decoupled from the agent, so it can be swapped without touching the logic, just by changing the identifier.

**Strict Structured Output Parser with auto-fix**: The agent's output is constrained to a strict JSON schema with `enum` for discrete choices (budget, fit, urgency), `minimum`/`maximum` for the 1-10 score, and `required` on all critical fields. The `autoFix: true` option with a sub-node model (OpenCode Go + DeepSeek v4 Flash) automatically corrects invalid schemas. The agent is forced to pass through lead metadata (leadName, leadEmail, leadCompany) in its output, ensuring complete contact traceability.
