...
L'instance n8n n'est plus un simple orchestrateur de tâches récurrentes. Deux workflows critiques y tournent en production, connectés par une **même Data Table CRM** — le cœur du système.
...
Agent WhatsApp qui permet de **lire** et interroger la même CRM en langage naturel — consulter les leads récents, chercher par email, mettre à jour un statut, envoyer un email professionnel. Mémoire Redis pour le contexte conversationnel.
...
<Callout type="tip">
  Chaque workflow a sa propre logique de sécurité : le Lead Qualification Agent utilise une credential Header Auth (`X-Webhook-Secret`) sur son webhook, tandis que le WhatsApp CRM Assistant s'appuie sur l'authentification WhatsApp Business de n8n. Les deux ont un `retryOnFail` configuré sur les nœuds critiques.
</Callout>
