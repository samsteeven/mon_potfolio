...
**La demande.** Je branche une IA sur mon serveur MCP et je lui colle ce prompt :

```text
Tu es connecté à mon second cerveau via MCP (outils : second_brain_ask, second_brain_project_details, second_brain_add).
...
Trois choses comptent dans ce prompt : **l'ordre des outils** (la base d'abord, la source seulement si besoin de détail), **l'interdiction d'inventer** (avec obligation de signaler les écarts), et **la demande de lister d'abord ce qui a été trouvé**. C'est cette dernière ligne qui produit les captures ci-dessous.
...
Ce que j'ai trouvé le plus utile n'est pourtant pas le CV. C'est la liste des écarts. L'IA y pointe les **manques réels de ma base** : des projets décrits trop vite (React seulement via SIGGE, sans détail de ce que j'y ai fait), un projet présenté comme un cas d'étude plutôt qu'une mission réelle, des compétences absentes (Kotlin, etcd, Operator Framework). Sa remarque sur les « fichiers tronqués » méritait vérification : elle venait moins de ma base que de mon moteur de recherche. Mon indexeur découpait les notes **en plein milieu des phrases**, sans titre ni contexte, et ne remontait que **huit morceaux par question**. Le contenu était souvent là, mais mal découpé, donc introuvable. Depuis que je découpe **un morceau par section** en le préfixant par sa source, des infos que je croyais absentes remontent en tête des résultats, comme **mon rôle sur TribuneJustice**. Le cerveau ne vaut que par ce qu'on y met, mais aussi par ce qu'il sait en retrouver. C'est un début, et il rend ces trous visibles.
