...
const Button = ({
    ...
    if (as === 'link') {
        ...
        if (props.target === '_blank') {
            // Only pass anchor-compatible props to <a>, avoid spreading Link props (ref, method, data, etc.)
            const anchorProps: AnchorHTMLAttributes<HTMLAnchorElement> = {
                href: props.href?.toString() ?? '#',
                target: props.target,
                rel: props.rel,
                // Add other safe anchor props here if needed (onClick, title, aria-*, etc.)
            };
